...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="css/admin-appointment[cancelled]-css.css">
    <style>
...
<div class="wrapper">
    ...
    $sql = "SELECT p.name, p.phone, p.email, p.gender, sc.scheduleDateTime, s.service_name, cc.chief_complaint, cc.details, cc.dentalpain_status, cc.dental_trauma, cc.bleeding_tissues, mh.medical_conditions, mh.diabetes, mh.heart_conditions, mh.hypertension, mh.maintenance, mh.systolic_pressure, mh.diastolic_pressure, GROUP_CONCAT(DISTINCT ma.medicine_name SEPARATOR ', ') AS medicine_name, GROUP_CONCAT(DISTINCT hs.symptom_name SEPARATOR ', ') AS symptom_name, pll.pain_value, hd.covidvaccine_status, pd.referenceno, pd.amount, pd.image_filename, a.appointment_id
            ...
            JOIN payment pd ON a.appointment_id = pd.paymentDetails_ID
            JOIN schedule sc ON a.appointment_id = sc.schedule_id
            ...
            WHERE p.patient_status = 'Cancelled'
            GROUP BY a.appointment_id
            ORDER BY sc.scheduleDateTime";
        ...
        ?>
                        <div class="dropdown">
                            ...
                            <div id="dropdownData-<?php echo $id; ?>-event" class="dropdown-content">
                                <table>
                                    <tr><th>Appointment Date:</th><td><?php echo $row["scheduleDateTime"]; ?></td></tr>
                                    ...
                                    <tr><th>Payment Details:</th><td><?php echo $row["referenceno"]; ?></td></tr>
									<tr><th>Amount:</th><td><?php echo $row["amount"]; ?></td></tr>
									<tr><th>Proof of Payment</th><td> <a href="images/<?php echo $row['image_filename']; ?>" target="_blank"><img src="images/<?php echo $row['image_filename']; ?>" alt="Proof of Payment" style="max-width: 50%; max-height: auto; cursor: pointer;" id="image-<?php echo $row['image_filename']; ?>"></a></td></tr>
                                </table>
                            </div>
                        </div>
                        <?php
